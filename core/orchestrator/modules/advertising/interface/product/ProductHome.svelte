<script>
  ...
  const flow = [
    ...
    ['2','Привести','Если данные уже есть — автоматически определить дату, сущности и метрики.','#assistant'],
  ...
  async function createContainer() {
    ...
    creating = true; error = '';
    try {
      const result = await productPost('/product/containers/upsert', { name: name.trim(), kind, description: '', status: 'active' });
...
<section class="home">
  ...
  <section class="containers">
    <div class="head"><div><small>Рабочий контекст</small><strong>Контейнер</strong></div><form on:submit|preventDefault={createContainer}><select bind:value={kind}><option value="client">Клиент</option><option value="process_group">Группа процессов</option><option value="project">Проект</option><option value="experiment">Эксперимент</option></select><input bind:value={name} placeholder="Новый контейнер"/><button disabled={creating || !name.trim()}>Создать</button></form></div>
    <div class="chips">
