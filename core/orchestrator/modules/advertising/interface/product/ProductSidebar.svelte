...
<style>
  ...
  .brand { display: flex; align-items: center; gap: 9px; padding: 4px 7px 20px; color: #172033; text-decoration: none; }.brand > b { width: 33px; height: 33px; display: grid; place-items: center; border-radius: 10px; background: #172033; color: #fff; font-size: 11px; }.brand span { display: flex; flex-direction: column; }.brand strong { font-size: 12px; }.brand small { color: #94a3b8; font-size: 9px; }
  ...
  @media (max-width: 760px) { aside { height: auto; z-index: 30; padding: 8px; border-right: 0; border-bottom: 1px solid #e4e9f1; } .brand, nav section > small, .status { display: none; } nav { flex-direction: row; gap: 4px; overflow-x: auto; } nav section { flex-direction: row; gap: 4px; } nav a { white-space: nowrap; padding: 7px 9px; } }
</style>
