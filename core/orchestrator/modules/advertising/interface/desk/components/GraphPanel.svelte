<script lang="ts">
  ...
  const edgeLabelGroup = new THREE.Group(); // подписи осей/рёбер
  ...
  function normalizeBBox(list: SpacePoint[]): BBox {
    if (!list.length) return { minX: -90, maxX: 90, minY: -90, maxY: 90, minZ: -90, maxZ: 90 };
  ...
  // =========================
  // SPRITE-ТЕКСТ (ДЛЯ ОСЕЙ)
  // =========================

  function makeTextSprite(text: string, opts: { fontSize?: number; padding?: number } = {}): THREE.Sprite {
    const { fontSize = 34, padding = 14 } = opts;
    const canvas = document.createElement('canvas');
    const ctx = canvas.getContext('2d');
    if (!ctx) {
      // fallback — пустая надпись
  ...
  function disposeEdgeLabels(): void {
    // аккуратно освобождаем текстуры у старых спрайтов
    edgeLabelGroup.children.forEach((obj) => {
  ...
  function calcMax(pointsList: SpacePoint[], metricKey: string): number {
    ...
    for (const p of pointsList) {
      ...
      if (Number.isFinite(v)) max = Math.max(max, v as number);
    }
  ...
  function formatValueByMetric(metricCode: string, value: number): string {
    if (!Number.isFinite(value)) return 'нет данных';

    const money = new Set(['revenue', 'spend', 'margin', 'profit']);
    if (money.has(metricCode)) return `${formatNumberHuman(value)} ₽`;
    ...
    if (metricCode === 'roi') return value.toFixed(2).replace('.', ',');

    return formatNumberHuman(value);
  }

  /**
   * Главная функция: подписи осей от пересечения плоскостей (min/min/min)
   * до конца ребра (max по соответствующей оси).
   */
  function updateEdgeLabels(params: {
    ...
    // точка пересечения плоскостей (угол куба)
    const origin = new THREE.Vector3(bbox.minX, bbox.minY, bbox.minZ);

    // концы осей по ребрам
    const xEnd = new THREE.Vector3(bbox.maxX, bbox.minY, bbox.minZ);
    const yEnd = new THREE.Vector3(bbox.minX, bbox.maxY, bbox.minZ);
    const zEnd = new THREE.Vector3(bbox.minX, bbox.minY, bbox.maxZ);

    // небольшие смещения, чтобы надписи не прилипали к плоскостям
    const off = 6;

    // X
    if (xMetric) {
      const xMax = calcMax(pointsList, xMetric);
      const sName = makeTextSprite(xName);
      sName.position.copy(origin).add(new THREE.Vector3(0, -off, +off));
      const sMax = makeTextSprite(formatValueByMetric(xMetric, xMax));
      sMax.position.copy(xEnd).add(new THREE.Vector3(0, -off, +off));
      edgeLabelGroup.add(sName, sMax);
    }

    // Y
    if (yMetric) {
      const yMax = calcMax(pointsList, yMetric);
      const sName = makeTextSprite(yName);
      sName.position.copy(origin).add(new THREE.Vector3(-off, 0, +off));
      const sMax = makeTextSprite(formatValueByMetric(yMetric, yMax));
      sMax.position.copy(yEnd).add(new THREE.Vector3(-off, 0, +off));
      edgeLabelGroup.add(sName, sMax);
    }
    ...
    if (zMetric) {
      ...
      const sName = makeTextSprite(zName);
      sName.position.copy(origin).add(new THREE.Vector3(-off, -off, 0));
      const sMax = makeTextSprite(formatValueByMetric(zMetric, zMax));
      sMax.position.copy(zEnd).add(new THREE.Vector3(-off, -off, 0));
      edgeLabelGroup.add(sName, sMax);
    }
  }

  // =========================
  // ПЛОСКОСТИ + ВЫЗОВ ПОДПИСЕЙ ОСЕЙ
  // =========================
  ...
  function rebuildPlanes(list: SpacePoint[]): void {
    ...
    // ВАЖНО: подписи осей от пересечения плоскостей
    updateEdgeLabels({
      bbox,
      pointsList: list,
      xMetric: axisX,
  ...
  function init3d(): void {
    ...
    // группы (плоскости + подписи рёбер)
  ...
  // =========================
  // UI: СЛОИ / ПОЛЯ / ФИЛЬТРЫ / НАСТРОЙКИ
  // =========================
