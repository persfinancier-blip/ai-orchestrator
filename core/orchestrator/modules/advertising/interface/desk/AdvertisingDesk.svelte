<script lang="ts">
  ...
  function formatCell(field: ShowcaseField | undefined, value: unknown): string {
    ...
    if (field.kind === 'number') {
      if (field.code === 'revenue' || field.code === 'spend') {
        ...
        <div>
          ...
          <ul>
            {#each textFields as field}
              <li>{field.name} <span>Текст</span></li>
            {/each}
          </ul>
        </div>
        ...
        <div>
          ...
          <ul>
            {#each numberFields as field}
              <li>{field.name} <span>Число</span></li>
            {/each}
          </ul>
        </div>
        ...
        <div>
          ...
          <ul>
            {#each dateFields as field}
              <li>{field.name} <span>Дата</span></li>
            {/each}
          </ul>
        </div>
      </div>
          ...
          <thead>
            <tr>
              {#each sampleColumns as column}
                <th>{getField(column)?.name ?? column}</th>
              {/each}
            </tr>
          </thead>
          <tbody>
