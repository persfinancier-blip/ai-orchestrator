...
<div class:table-node-builder={true} class:table-node-builder-embedded={embeddedMode}>
  <section class="shell-section shell-section-compact">
    <div class="shell-head compact-head">
      <div>
        <h3>Входящие параметры</h3>
        <p>Зона входных параметров. На этом шаге без привязок и логики.</p>
      </div>
    </div>
    <div class="shell-placeholder">Пока нет данных для отображения входных параметров.</div>
  </section>
  ...
  <section class="builder-card">
    ...
    <div class="preview-metrics"><span>Строк на входе: {previewData?.input_row_count ?? '-'}</span><span>Колонок на входе: {previewData?.input_column_count ?? '-'}</span><span>Источник: {previewData?.source_ref || '-'}</span><span>Обновлено: {previewUpdatedAt ? new Date(previewUpdatedAt).toLocaleString('ru-RU') : '-'}</span></div>
    {#if inputPreviewColumns.length}<div class="preview-columns">{#each inputPreviewColumns as column}<span>{column}</span>{/each}</div>{/if}
    {#if inputPreviewRows.length}<div class="preview-table-wrap"><table class="preview-table"><thead><tr>{#each inputPreviewColumns as column}<th>{column}</th>{/each}</tr></thead><tbody>{#each inputPreviewRows as row}<tr>{#each inputPreviewColumns as column}<td>{typeof row?.[column] === 'object' ? JSON.stringify(row?.[column]) : String(row?.[column] ?? '')}</td>{/each}</tr>{/each}</tbody></table></div>{:else}<div class="empty-box">Preview входа появится после выбора основной таблицы и запуска server-side preview.</div>{/if}
  </section>
