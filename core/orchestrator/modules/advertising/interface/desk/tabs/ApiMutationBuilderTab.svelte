<script>
  ...
  function normalizeSettings(raw) { const src = raw && typeof raw === 'object' ? raw : {}; return { endpointUrl: str(src.endpointUrl || src.endpoint_url || src.apiUrl || src.api_url), httpMethod: str(src.httpMethod || src.http_method || src.apiMethod || src.api_method || 'POST').toUpperCase() || 'POST', authMode: str(src.authMode || src.auth_mode || 'manual').toLowerCase() === 'bearer_token' ? 'bearer_token' : 'manual', authJson: pretty(src.authJson || src.auth_json || {}, '{}'), headersJson: pretty(src.headersJson || src.headers_json || src.apiHeaders || src.api_headers || { 'Content-Type': 'application/json' }, '{}'), queryJson: pretty(src.queryJson || src.query_json || src.apiQuery || src.api_query || {}, '{}'), bodyJson: pretty(src.bodyJson || src.body_json || src.apiBody || src.api_body || {}, '{}'), bodyItemsPath: str(src.bodyItemsPath || src.body_items_path || 'items') || 'items', bindingRulesJson: pretty(src.bindingRules || src.bindingRulesJson || src.binding_rules || [], '[]'), responseMappingsJson: pretty(src.responseMappings || src.responseMappingsJson || src.response_mappings || [], '[]'), requestMode: str(src.requestMode || src.request_mode || 'row_per_request').toLowerCase() === 'batch_request' ? 'batch_request' : 'row_per_request', batchSize: str(src.batchSize || src.batch_size || '50') || '50', dryRun: String(src.dryRun ?? src.dry_run ?? 'true').trim().toLowerCase() === 'false' ? 'false' : 'true', maxRowsPerRun: str(src.maxRowsPerRun || src.max_rows_per_run || '500') || '500', retryCount: str(src.retryCount || src.retry_count || '1') || '1', timeoutMs: str(src.timeoutMs || src.timeout_ms || '15000') || '15000', stopPolicy: str(src.stopPolicy || src.stop_policy || 'stop_on_error').toLowerCase() === 'continue' ? 'continue' : 'stop_on_error', previewLimit: str(src.previewLimit || src.preview_limit || '20') || '20', channel: str(src.channel || '') }; }
  ...
  $: bindingRules = parseJson(settings.bindingRulesJson, []).map((item, index) => normalizeBinding(item, index));
  $: responseMappings = parseJson(settings.responseMappingsJson, []).map((item, index) => normalizeResponse(item, index));
